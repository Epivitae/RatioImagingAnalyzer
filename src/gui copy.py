...
class RatioAnalyzerApp:
    ...
    def setup_theme(self):
        ...
        style.configure("TButton", padding=5, font=self.f_normal) # Increased padding for consistent height
        ...
        # Modern Button Styles - Uniform padding is key
        ...
        # Toolbutton for Shapes (Flat look)
    ...
    def setup_file_group(self):
        ...
        # [Fixed] Aligned margins for the Interleaved button
        self.chk_inter = ttk.Checkbutton(self.tab_dual, variable=self.is_interleaved_var, style="Toggle.TButton")
        self.chk_inter.pack(fill="x", pady=(2, 0)) # Reduced top padding to match button proximity
    ...
    def create_bottom_panel(self, parent):
        ...
        tb_frame = ttk.Frame(row_ctl, style="White.TFrame")
        tb_frame.pack(side="right")
        
        # [优化] 延后 50ms 加载工具栏，优先渲染下方的 ROI 面板，解决视觉卡顿
        self.root.after(50, lambda: self.plot_mgr.add_toolbar(tb_frame))
        ...
        # 注意：这里去掉了 self.ui_elements["btn_clear"] 以防止翻译覆盖图标
    ...
    def load_data(self):
        try:
            ...
            self.plot_mgr.init_image((h, w), cmap="jet")
